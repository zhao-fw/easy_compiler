/************************************************************************
* 定义了LR1类															*
*	主要任务：根据输入的grammer生成分析表（ACTION和GOTO表）				*
*																		*
* 定义结构体：															*
* 	symbol：产生式的每个符号（空符号epsilon使用 '@' 表示）				*
* 	production：产生式													*
* 	f_set：求first、follow集合时的辅助类								*
* 	Item：项目（产生式 + 圆点（·） + 一个展望的符号）					*
* 	Item_set：项目集（上述项目的集合，也就是状态机的一个节点）			*
*																		*
* 分析过程：															*
*	读grammer，初始化symbol和production									*
*	计算first集合和follow集合（follow集合其实不需要）					*
*	生成DFA（GO函数）													*
*	生成最终需要的分析表（ACTION表和GOTO表）							*
************************************************************************/
...
public:
	...
	int find_in_symbol(const char* c);						/* 在已有符号表中查找 */
	void append_symbol(char* c);							/* 添加新的符号 */
	void append_symbol(const char* c);						/* 添加新的符号 */
	int find_in_production(char* c);						/* 在已有的产生式中寻找 */
	int find_in_production(const char* c);					/* 在已有的产生式中寻找 */
	void append_item(Item one);								// 产生某个项目
	void append_item(int p_id, int location, int outlook);	// 产生某个项目
	int find_item(Item one);								// 判断某个项目已经存在，找到返回id号，否则返回-1

...
public:
	...
	// 返回四元式
	IntermediateCode& getIntermediateCode() { return INTcode; }
	vector<vector<tblEntry>>& getVarTable() { return varTable; }
	vector<funcEntry>& getFuncTable() { return funcTable; }
};
...
SYM Smantic::Smantic_analyze(string s, int left_id, vector<SYM> node)
{
	...
	if (s._Equal("<N> -> @")) {
		...
		left_node = SYM(left_id, "", 0);
		//注意N还没有处理
		//假如是有main函数的情况，可以在此处回填, 也就是程序一开始就执行main程序
	...
	/* 别忘了空产生式 */
		...
		varName.push_back(func.get_name());

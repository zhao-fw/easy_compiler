/************************************************************************
*	定义了符号类，是语法树的每个节点（终结与非终结符）					*
*		作用：记录符号的属性（type、name）								*
/************************************************************************/
...
private:
	...
	char symbol_name[20] = { 0 };	// 前后没有配合好的问题...跟name内容相同，不过使用的类型不同，有时间重构代码
	string name;					// 标识符的名称(只有标识符需要)
	int place;						// 符号在符号表中的位置
	int line;						// 符号的行数，与Lexer中对应
	int quad;						// M, N的空产生式需要考虑回填
